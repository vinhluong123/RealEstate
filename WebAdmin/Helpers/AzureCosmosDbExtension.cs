...
{
    public class AzureCosmosDbExtension
    {
        ...
        private string databaseId = "TestSampleDB";
        private string containerId = "TestSampleContainer";


        public AzureCosmosDbExtension(IConfiguration configuration)
        {
            Configuration = configuration;
            EndpointUri = Configuration.GetSection("CosmosDb")["EndpointUri"];
            PrimaryKey = Configuration.GetSection("CosmosDb")["PrimaryKey"];
        }
        public async Task TestAzureCosmosDb()
        {
            try
            {
        ...
        public async Task GetStartedDemoAsync()
        {
            ...
            this.cosmosClient = new CosmosClient(EndpointUri, PrimaryKey, new CosmosClientOptions() { ApplicationName = "CosmosDBDotnetLhvinh" });
            await this.CreateDatabaseAsync();
            await this.CreateContainerAsync();
            await this.ScaleContainerAsync();
            await this.AddItemsToContainerAsync();
            await this.QueryItemsAsync();
            dynamic newvalue = new
            {
                LastName = "NewValue2",
                IsRegistered = false,
                Address = new
            ...
            await this.UpdateObjectItemAsync("Andersen.1", "Andersen", newvalue);
        ...
        private async Task UpdateObjectItemAsync<T>(string id, string name, string newvalue)
        {
            ItemResponse<T> familyResponse = await this.container.ReadItemAsync<T>(id, new PartitionKey(name));
            var itemBody = familyResponse.Resource;
            // Set new value for item body here
            var itemType = typeof(T);
            var property = itemType.GetProperty("value");
            if (property!= null)
            {
                property.SetValue(itemBody, newvalue);
            }
            else
            {
                throw new ArgumentException("The provided type does not have a property named 'value'.");
            }
            await this.container.ReplaceItemAsync(itemBody, id, new PartitionKey(name));
            Console.WriteLine($"Item updated successfully. ID: {id}, Name: {name}");
        }

        private async Task UpdateObjectItemAsync(string id, string name, dynamic newValues)
        {
            try
            {
                // Read the item from Cosmos DB
                ItemResponse<dynamic> itemResponse = await this.container.ReadItemAsync<dynamic>(id, new PartitionKey(name));
                var itemBody = itemResponse.Resource;

                // Update top-level properties
                foreach (var property in newValues.GetType().GetProperties())
                {
                    var propertyName = property.Name;
                    var propertyValue = property.GetValue(newValues);

                    // Check if the property exists in the item body
                    if (itemBody.ContainsKey(propertyName))
                    {
                        // If the property is an object, recursively update its properties                        
                        if (propertyValue.GetType().IsClass && propertyValue.GetType() != typeof(string))
                        {
                            UpdateNestedProperties(itemBody[propertyName], propertyValue);
                        }
                        else
                        {
                            // Update the property in the item body
                            itemBody[propertyName] = propertyValue;
                        }
                    }
                }

                // Replace the item in Cosmos DB with the updated item
                await this.container.ReplaceItemAsync(itemBody, id, new PartitionKey(name));
        ...
        private void UpdateNestedProperties(dynamic item, dynamic newValues)
        {
            foreach (var property in newValues.GetType().GetProperties())
            {
                var propertyName = property.Name;
